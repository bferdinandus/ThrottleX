...
public class LoconetClient : IDisposable
{
    ...
    /// </summary>
    public enum ReceiveOrigin
    {
        /// <summary>
        /// Somebody on the network sent this
        /// </summary>
        FromNet,
        /// <summary>
        /// Somebody on the network sent this, but it is a reply to what we sent
        /// </summary>
        Reply,
        /// <summary>
        /// We sent this to the net
        /// </summary>
        Sent,
    }
    ...
    /// </summary>
    public event Action<ReceiveOrigin, ReceivableLoconetMessage>? OnMessageReceived;

    public LoconetClient(int index, string host, ushort port, ILogger logger)
    {
        Index = index;
        Host = host;
        Port = port;
    ...
    protected virtual void OnReceive(string param)
    {
        ...
        if (_nextReceiveIsReply && msg[0]!=0x81)
        {
            ...
            _reply = parsed;
            _replyEvent.Set();
            educatedGuess = ReceiveOrigin.Reply;
        }
        else if (param.Equals(_lastSentHex))
        {
            educatedGuess = ReceiveOrigin.Sent;
            _lastSentHex = null;
        }
        else
        {
            educatedGuess = ReceiveOrigin.FromNet;
        }

        OnMessageReceived?.Invoke(educatedGuess, parsed!);
    }
